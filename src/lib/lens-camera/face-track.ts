 ...
 * Fallback: lightweight heuristic tracker when MediaPipe fails to init
 *
...
function landmarksFromMediaPipe(
  ...
  // MediaPipe Face Mesh indices (selected key points)
  const L_EYE = 33;
  const R_EYE = 263;
  const NOSE = 1;
  const MOUTH = 13;
  ...
  // Approximate head-top slightly above forehead
  ...
  const scale = eyeDist / 0.12; // normalize around typical face
...
/** Lightweight heuristic fallback (no ML) */
function heuristicTrack(
  video: HTMLVideoElement,
  canvas: HTMLCanvasElement
): FaceLandmarks | null {
  // Very rough center-face estimate when MediaPipe unavailable
...
export async function detectFace(
  ...
  if (!fl) {
    return canvas ? heuristicTrack(video, canvas) : null;
  }

  try {
    // MediaPipe requires monotonically increasing timestamps
    videoTimestamp += 33; // ~30fps
