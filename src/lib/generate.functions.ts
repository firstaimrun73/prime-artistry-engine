...
export const generateMedia = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const isCircleOp =
        data.type === "image" && data.maskImageUrl != null && data.circleInstant !== undefined;
    ...
    if (reservedVideoQuote) {
      ...
      if (dErr || !deduction) {
        if (dErr?.message?.includes("INSUFFICIENT_CREDITS")) {
          throw new Error(
            `Not enough credits. ${data.type === "video" ? "Video" : "Image"} generation costs ${chargeAmount} credits.`,
          );
        }
