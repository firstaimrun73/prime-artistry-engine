...
export const generateMedia = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Enforce Standard / Premium / Ultra experience entitlement server-side.
    if (data.type === "image") {
      ...
      : data.type === "video"
        ...
        : useStandardImagePath(data.studioTier)
          ? (() => {
              ...
              // Total images = primary + refs (GPT Image 2 multi table uses total)
              const totalImages =
          ...
          : isPremiumMultiGptCandidate({
            ...
            ? (() => {
                ...
                if (!planned.ok) {
                  // Fall through cost only if not multi; else use error floor
  ...
  : (() => {
    ...
    if (data.type === "image" && useStandardImagePath(data.studioTier)) {
      ...
      // ——— PREMIUM (pro) multi-reference GPT Image 2 @ quality low ———
      const planned = planPremiumMultiGptImage2({
        prompt: data.prompt,
        imageUrl: data.imageUrl,
        referenceImageUrls: data.referenceImageUrls,
    ...
    // Persist real HTTPS source URL for I2I (never the placeholder "uploaded").
