...
export const generateMedia = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // History MUST use service-role client. User-scoped client often fails RLS
    // on insert; silent log-only failure was the root cause of empty History.
    const historyRow = {
      user_id: userId,
      type: data.type,
      prompt: data.prompt,
    ...
    const { error: historyErr } = await supabaseAdmin.from("generations").insert(historyRow);
    if (historyErr) {
      console.error(
