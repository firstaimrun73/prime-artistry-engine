  ...
  .handler(async ({ data, context }) => {
    ...
    if (!isAdmin) {
      if (data.type === "video" && reservedVideoQuote) {
        ...
        const { data: updated, error: cErr } = await supabaseAdmin
          ...
          .select("credits")
          .single();
        if (cErr) throw new Error("Could not update credits.");
        newCredits = updated?.credits ?? profile.credits - charge;
      }
    }
    ...
    try {
      await supabaseAdmin.from("generation_history").insert({
        ...
        credits_used: isAdmin ? 0 : standardCharge ?? cost,
        meta: circleAddHistoryMeta ?? null,
      });
    } catch (histErr) {
