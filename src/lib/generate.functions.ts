...
function safePayload(body: Record<string, unknown>): Record<string, unknown> {
  ...
  for (const [k, v] of Object.entries(body)) {
    out[k] = typeof v === "string" && v.length > 80 ? `${v.slice(0, 64)}… (${v.length} chars)` : v;
  }
  return out;
}

function isPixelLimitError(msg: string): boolean {
  return /4095|4096|maximum should be|too large|max(imum)?\s*(image\s*)?(size|dimension|pixels)|exceeds|resolution too high/i.test(msg || "");
}
...
const inputSchema = z.object({
  ...
  videoDurationSeconds: z.union([z.literal(5), z.literal(10), z.literal(15), z.literal(20), z.literal(25), z.literal(30)]).optional(),
...
export const generateMedia = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const videoDuration = isAdmin ? requestedDuration : (Math.min(requestedDuration, maxDuration) as typeof requestedDuration);
    const isVideoEnhance = data.type === "video" && data.sourceKind === "video";
    const CIRCLE_INSTANT_CREDITS = 35;
    ...
    await supabase.from("generations").insert({
      ...
      metadata: data.type === "video" ? { duration: data.videoDurationSeconds ?? 5, resolution: data.videoResolution ?? null, aspect_ratio: data.videoAspectRatio ?? null } : null,
