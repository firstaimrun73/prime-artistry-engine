...
export const generateMedia = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Video: server-authoritative quote (provider COGS → MotioCreditPricingEngine).
    // Image paths keep existing experience credit helpers until Phase 18.
    ...
    let cost = isVideoEnhance
      ...
      : data.type === "video"
        ? (() => {
            ...
            // Product mode is a duration/quality ceiling, not a model pool.
            const productMode: VideoProductMode =
              videoDuration > 10 || (data.videoResolution === "1080p")
                ? "premium"
                : "standard";
            ...
            const audioRequested =
              data.videoGenerateAudio === true ||
              promptMentionsAudio(data.prompt || "");
    ...
    try {
      if (data.type === "image" && useStandardImagePath(data.studioTier)) {
        ...
        // Server selects approved economical route — never trust client videoModelId for policy.
        const mode: VideoGenMode =
          data.sourceKind === "video" ? "video" : data.imageUrl ? "image" : "text";
        const productMode: VideoProductMode =
        ...
        const aspect = (data.videoAspectRatio as VideoAspect) || "16:9";
        const audioRequested =
          data.videoGenerateAudio === true || promptMentionsAudio(data.prompt || "");
        ...
        if (route) {
          step = buildVideoFromRegistry({
            ...
            prompt: styledPrompt,
            imageUrl: data.imageUrl,
            videoUrl: mode === "video" ? data.imageUrl : undefined,
            durationSeconds: videoDuration,
            aspectRatio: aspect,
            resolution,
            generateAudio: audioRequested && route.model.nativeAudio,
            mode,
          });
        ...
        if (!step) throw new Error("No video pipeline selected for these settings.");
        outputUrl = await runFalStepResilient(step, falKey);
      }
    } catch (err) {
    ...
    try {
      ...
      const isCircleOp = data.type === "image" && data.maskImageUrl != null && data.circleInstant !== undefined;
      const finalized = await finalizeMediaAsset({
        ...
        userId,
        studioTier: data.studioTier,
        watermarkBrand: isCircleOp ? "circle" : "generic",
      });
    ...
    const historyMetadata =
      data.type === "video"
        ? {
            ...
            model: videoQuote?.ok ? videoQuote.quote.modelId : (data.videoModelId ?? null),
            quote_id: videoQuote?.ok ? videoQuote.quote.quoteId : null,
            pricing_version: videoQuote?.ok ? videoQuote.quote.pricingVersion : null,
            provider_cogs_usd: videoQuote?.ok ? videoQuote.quote.providerCogsUsd : null,
          }
        : {
    ...
    try {
      ...
      console.error("[generate] history insert failed:", histErr);
      // Soft-fail: media and credits already finalized. Do not block the user result.
    }
