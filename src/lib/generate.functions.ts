...
export const generateMedia = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // Circle Add: server resolves authoritative backendPrompt from circleAssetId.
    // Remove path (circleInstant true) is unchanged.
    ...
    if (data.type === "image" && data.circleInstant === false) {
      ...
      const resolved = resolveCircleAddPrompt({
    ...
    if (data.type === "image" && useStandardImagePath(data.studioTier)) {
      ...
      throw new Error("Video path temporarily unavailable in this restore; please retry.");
    }
    } catch (err) {
