...
export const generateMedia = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const historyMetadata =
      data.type === "video"
        ...
        : {
            ...
            ...(data.maskImageUrl
              ? {
                  circle_operation: data.circleInstant === true ? "remove" : data.circleInstant === false ? "add" : "mask_edit",
    ...
    // History MUST use service-role. metadata column may be missing on older DBs —
    // fall back without it so jobs still appear in History.
    const historyBase = {
      user_id: userId,
      type: data.type,
      prompt: data.prompt,
    ...
    const historyMeta = {
      ...
      operation: data.type === "video"
        ? (isVideoEnhance ? "video_enhance" : "video_generate")
        : data.maskImageUrl
          ? data.circleInstant === true
            ? "circle_remove"
            : data.circleInstant === false
              ? "circle_add"
              : "mask_edit"
          : data.imageUrl
            ? "image_edit"
    ...
    let historyErr = (await supabaseAdmin.from("generations").insert({ ...historyBase, metadata: historyMeta })).error;
    if (historyErr && /metadata|column|schema/i.test(historyErr.message + (historyErr.details ?? ""))) {
      console.warn("[generate] metadata column missing — retrying without metadata");
      historyErr = (await supabaseAdmin.from("generations").insert(historyBase)).error;
    }
    if (historyErr) {
      console.error(
        "[generate] history insert failed:",
        historyErr.message,
        historyErr.code,
        historyErr.details,
        historyErr.hint,
      ...
      console.log("[generate] history saved for user", userId);
    }
