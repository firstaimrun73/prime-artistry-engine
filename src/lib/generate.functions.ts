...
const inputSchema = z.object({
  ...
  /** Frontend @tag ids — expanded server-side into model prompt only; never changes image URLs. */
...
export const generateMedia = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    // @tags → detailed semantic instructions for the model only.
    // imageUrl / referenceImageUrls / routing / credits are NEVER changed by tags.
    const modelPrompt =
      data.type === "image"
