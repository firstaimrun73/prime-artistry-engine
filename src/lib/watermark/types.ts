...
export type WatermarkPolicy = {
  mode: WatermarkMode; primary: boolean; secondary: boolean;
  alreadyFinalized: boolean; reason: string;
};
export type FinalizeMediaInput = {
  sourceUrl?: string; sourceBuffer?: Buffer; mediaKind: MediaKind;
  plan: string | null | undefined; email?: string | null | undefined;
  isAdmin?: boolean; keepWatermark?: boolean; userId: string;
  alreadyFinalizedHint?: boolean;
};
export type FinalizeMediaResult = {
  finalUrl: string; watermarked: boolean; mode: WatermarkMode;
  storagePath?: string; skippedAsFinalized: boolean;
