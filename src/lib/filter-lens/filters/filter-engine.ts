...
function applyMonochromeSepia(data: Uint8ClampedArray, mono: boolean, sepia: number) {
  ...
  for (let i = 0; i < data.length; i += 4) {
    ...
    if (mono) {
      // Pure neutral grayscale — no residual color cast
      data[i] = clamp8(g);
      data[i + 1] = clamp8(g);
      data[i + 2] = clamp8(g);
    } else if (t > 0) {
...
/** True comic/illustration: flat posterized color + hard black ink contours. */
function applyComicStyle(image: RGBAImage, intensity: number) {
  const w = image.width, h = image.height;
  const src = new Uint8ClampedArray(image.data);
  const t = Math.max(0.35, Math.min(1, intensity / 100));
  applySoftBlur(image, 12 + t * 8);
  applyPosterize(image.data, 4);
  applySaturationVibrance(image.data, 28 + t * 18, 16 + t * 10);
  applyContrastish(image.data, 18 + t * 12);
  const edgeThresh = 48 - t * 18;
  for (let y = 1; y < h - 1; y++) {
    for (let x = 1; x < w - 1; x++) {
      let gx = 0, gy = 0;
      for (let c = 0; c < 3; c++) {
      ...
      if (mag > edgeThresh) {
        const i = (y * w + x) * 4;
        const ink = Math.max(0, 255 - mag * (1.1 + t * 0.6));
        image.data[i] = ink;
        image.data[i + 1] = ink;
        image.data[i + 2] = ink;
      }
