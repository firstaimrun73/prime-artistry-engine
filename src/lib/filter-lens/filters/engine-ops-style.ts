...
function applyGhibli(img: ImgBuf, intensity: number): ImgBuf {
  // Soft painterly color palette + warm storybook light (free-tier local path).
  // Slightly higher oil radius / blend for more detail within free limits.
  const t = norm(intensity);
  const stats = computeImageStats(img);
  const radius = stats.edgeDensity > 0.12 ? 4 : 5;
  const painterly = oilPaint(img, radius, 9);
  let out = blend(img, painterly, 0.78 + t * 0.18);
  out = bilateralApprox(out, 3, 24);
  out = softQuantize(out, 12, 0.26);
  out = warmGreenLift(out, 0.6 + t * 0.4);
...
function applyRangoli(img: ImgBuf, intensity: number): ImgBuf {
  // Hyper-vibrant jewel-tone explosion (before → after). Keeps composition;
  // pushes magenta/teal/gold/violet/emerald saturation + glossy reflections.
  const t = norm(intensity);
