...
export function estimateModelCredits(opts: {
  ...
  const { model, durationSec, soundOn, mode, tier } = opts;
  if (!model.available) return 0;
  const d = Math.min(Math.max(1, durationSec), model.maxDuration);
  const isPremium = (tier ?? model.tier) === "premium";
  const isV2V = mode === "video";
  let base: number;
  if (isV2V) {
    base = soundOn && model.nativeAudio ? MIN_VIDEO_TO_VIDEO_CREDITS_SOUND : MIN_VIDEO_TO_VIDEO_CREDITS;
  } else if (isPremium) {
    base = soundOn && model.nativeAudio ? MIN_PREMIUM_VIDEO_CREDITS_SOUND : MIN_PREMIUM_VIDEO_CREDITS;
  } else {
...
export const VIDEO_MODELS: VideoModelDef[] = [
  ...
  },
  {
    ...
    name: "Veo 3.1",
    tier: "premium",
    bestUse: "Dialogue, lip-sync, true audio",
  ...
  },
  {
    ...
    name: "Seedance 2.0",
    tier: "premium",
    bestUse: "Cinematic physics + multi-shot audio + ultrawide",
  ...
  },
  {
    ...
    name: "LTX 2.3 Pro",
    tier: "premium",
    bestUse: "Open-weights flexibility",
...
export function capabilitiesForTier(tier: VideoTier, mode: VideoGenMode): {
  ...
  const resOrder: VideoResolution[] = ["480p", "720p", "1080p", "2k"];
  return {
