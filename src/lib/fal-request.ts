...
//
// Design:
// - Text to Image: FLUX1.1 [pro]
// - Image to Image: deterministic enhancement OR Kontext edit
...
const OUTFIT_MATCH =
  /\b(outfit|clothing|clothes|dress|shirt|jacket|armor|armour|costume|suit|wear|wearing|change\s+the\s+outfit|replace\s+outfit|swap\s+outfit|put\s+on|dress\s+(him|her|them)|clothes\s+from|PRIMARY TASK|OUTFIT TRANSFER)\b/i;
...
export function buildImageEdit({
  ...
  // Map UI strength (0.1-1) onto guidance when no explicit override
  let mappedGuidance = quality.guidance_scale;
  if (typeof strength === "number" && strength >= 0.1 && strength <= 1 && guidanceOverride === undefined) {
    mappedGuidance = 1.5 + strength * 3.5;
  }
  ...
  const alreadyBuilt =
    /PRIMARY TASK|OUTFIT TRANSFER|This is a real photograph/i.test(prompt);
  ...
  let guidance = guidanceOverride ?? mappedGuidance;
  ...
  return {
    label: `edit (flux kontext${isMulti ? ` multi x${refs.length + 1}` : ""}, ${editSize}/${editType})`,
    model,
    endpoint: ep(model),
    outputKind: "image",
    body,
