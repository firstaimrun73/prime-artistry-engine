...
export const DISCOVER_CATALOG: DiscoverItem[] = [
  ...
  {
    id: "circle-add-element",
    title: "Add element in place",
    description: "Mask a region and describe what to add.",
    category: "circle",
    tool: "circle",
    previewUrl: U.nature,
    prompt:
      "Add the described element into the masked region with realistic scale, lighting, and shadows so it blends naturally.",
    inputRequirement: "image-required",
    inputCount: 1,
    badge: "Circle 2edit",
    estimatedCredits: 10,
...
export function searchDiscover(query: string): DiscoverItem[] {
  ...
  return DISCOVER_CATALOG.filter((i) => {
    ...
    const hay = [i.title, i.description, i.badge, i.styleLabel, i.category, i.prompt]
      .filter(Boolean)
      .join(" ")
      .toLowerCase();
...
export function discoverTargetPath(item: DiscoverItem): string {
  switch (item.tool) {
    case "circle":
      return "/studio/image/circle-remove";
    case "auto-edit":
