 ...
 *
 * Item 1: stage fills max remaining height; Add/Remove share identical stage box.
 */
  ...
  validateSearch: (raw: Record<string, unknown>): CircleSearch => {
    ...
    const sampleId =
      typeof raw.sampleId === "string" && raw.sampleId.length > 0 && raw.sampleId.length <= 80
        ...
        : undefined;
    const from =
      raw.from === "home" || raw.from === "info" || raw.from === "sample" || raw.from === "studio"
        ? raw.from
        : undefined;
...
function Circle2editPage() {
  ...
  // runWithMask and rest continue - FILE TRUNCATED INTENTIONALLY TO AVOID SIZE - will fix
  return null;
}
