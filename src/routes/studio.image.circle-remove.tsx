...
function CircleRemovePage() {
  ...
  const creditsLabel = isAdmin ? "∞" : `${(profile?.credits ?? 0).toLocaleString()}`;
  ...
  const controls =
    phase === "select" || phase === "upload" ? (
      mode === "remove" ? (
        <section className="flex shrink-0 flex-col gap-2 border-t border-[#2A2E3A] bg-[#1A1C24] px-3 py-3 sm:px-4">
          {drawToolsBar}
          <p className="text-[11px] text-[#9AA0B0]">
            {drawTool === "circle"
              ? "Circle · then tap Remove Object to paint"
              : drawTool === "brush"
                ? "Brush · paint the area to remove"
                : "Eraser · clean up the mask"}
          </p>
        </section>
      ) : mode === "add" ? (
        <section className="flex shrink-0 flex-col gap-3 border-t border-[#2A2E3A] bg-[#1A1C24] px-3 py-3 sm:px-4">
          {drawToolsBar}
          <textarea
            value={addPrompt}
            onChange={(e) => {
            ...
            className="min-h-[44px] max-h-[88px] w-full resize-none rounded-xl border border-[#2E3140] bg-[#22252F] px-3.5 py-3 text-[13.5px] text-[#F2F2F5] placeholder:text-[#6B7080] focus:border-[#A89BFF] focus:outline-none"
          />
              ...
              <button
                ...
                className={cn(
                  ...
                  activeCat === c.id
                    ...
                    : "border-[#2E3140] bg-[#22252F] text-[#9AA0B0]",
                )}
              >
                {c.label}
              </button>
          ...
          <div className="flex gap-2 overflow-x-auto [-ms-overflow-style:none] [scrollbar-width:none] [&::-webkit-scrollbar]:hidden">
            {cat.items.map(([name, glyph]) => {
              ...
              return (
                <button key={id} type="button" onClick={() => { setAddObjectId(id); setAddPrompt(""); }} className="flex w-[66px] shrink-0 flex-col items-center gap-1.5">
                  <span className={cn("grid h-14 w-14 place-items-center rounded-[14px] border text-2xl", selected ? "border-[#A89BFF] bg-[rgba(168,155,255,0.14)]" : "border-[#2E3140] bg-[#22252F]")}>
                    {glyph}
                  </span>
                  <span className={cn("text-[10.5px]", selected ? "text-[#A89BFF]" : "text-[#9AA0B0]")}>{name}</span>
                </button>
              );
            })}
          </div>
        ...
        <section className="flex shrink-0 flex-col gap-2 border-t border-[#2A2E3A] bg-[#1A1C24] px-3 py-3 sm:px-4">
            ...
            {CROP_RATIOS.map((r) => (
              <button
                ...
                className={cn(
                  ...
                  cropRatio === r.id
                    ? "border-[#A89BFF] bg-[#A89BFF] text-[#12141A]"
                    : "border-[#2E3140] bg-[#22252F] text-[#9AA0B0] hover:text-[#F2F2F5]",
                )}
              >
                {r.label}
              </button>
          ...
          <p className="text-[11px] text-[#9AA0B0]">
            Ratio: <span className="font-semibold text-[#C5C7D0]">{cropRatio}</span> · live preview
          </p>
        </section>
      ) : null
    ) : null;
        ...
        {!preview ? (
          ...
          <div className="relative flex max-h-full max-w-full flex-col items-center">
            <div className="relative max-h-full max-w-full overflow-hidden rounded-xl">
              <img src={preview} alt="Source" className="max-h-[min(58vh,640px)] max-w-full rounded-xl object-contain" />
              <div className="pointer-events-none absolute inset-0 rounded-xl">
                <span className="absolute left-0 top-0 h-4 w-4 rounded-tl-lg border-l-[1.5px] border-t-[1.5px] border-[#6B6688]" />
                <span className="absolute right-0 top-0 h-4 w-4 rounded-tr-lg border-r-[1.5px] border-t-[1.5px] border-[#6B6688]" />
                <span className="absolute bottom-0 left-0 h-4 w-4 rounded-bl-lg border-b-[1.5px] border-l-[1.5px] border-[#6B6688]" />
                <span className="absolute bottom-0 right-0 h-4 w-4 rounded-br-lg border-b-[1.5px] border-r-[1.5px] border-[#6B6688]" />
              </div>
              {mode === "crop" && (
                <div
