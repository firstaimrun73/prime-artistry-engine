  ...
  return (
    <div className={cn("flex h-[100dvh] flex-col overflow-hidden", isDark ? "bg-gradient-to-b from-[#12141A] via-[#14161E] to-[#101218] text-[#F2F2F5]" : "bg-gradient-to-b from-[#F7F8FB] via-[#F2F3F7] to-[#EEEFF4] text-[#1A1C24]")} data-circle-2edit="true" data-theme={theme}>
      <header className={cn("flex shrink-0 items-center gap-2.5 border-b px-3 py-2.5 sm:gap-3 sm:px-4 sm:py-3", isDark ? "border-white/8 bg-[#181A22]/70 backdrop-blur-xl" : "border-black/[0.05] bg-white/55 backdrop-blur-xl")}>
        <button type="button" onClick={onBack} aria-label="Back to Studio" className={cn("grid h-9 w-9 shrink-0 place-items-center rounded-xl border backdrop-blur-md", isDark ? "border-white/10 bg-white/5 text-[#9AA0B0]" : "border-black/8 bg-white/70 text-[#5C6170]")}><ArrowLeft className="h-4 w-4" /></button>
...
/** Freehand irregular outline around an object — not a perfect geometric circle. */
...
/** Classic paint brush — bristles + ferrule + handle. */
...
/** Classic rectangular rubber eraser block — distinct from brush. */
function ToolIconEraser({ active }: { active: boolean }) {
  return (
    <svg viewBox="0 0 24 24" width="18" height="18" aria-hidden>
      ...
      <path d="M7 21.2h6" fill="none" stroke="currentColor" strokeWidth="1.1" strokeLinecap="round" opacity={0.4} />
    </svg>
  );
}

export function CircleDrawToolbar({ tool, onTool, brushSize, onBrushSize, hideCircle }: { tool: CircleDrawTool; onTool: (t: CircleDrawTool) => void; brushSize: number; onBrushSize: (n: number) => void; hideCircle?: boolean }) {
  const { theme } = useTheme();
  const isDark = theme === "dark";
