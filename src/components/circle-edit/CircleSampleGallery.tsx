 ...
 * Info (i) → /studio/image/circle-info with search.sampleId (real CIRCLE_SAMPLES id).
 * Try Now → circle-remove with from=home so back returns home.
 * IMPORTANT: never put ?query in `to` — TanStack Router needs search={{}}.
 */
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-3.5 sm:p-4">
        <div className="min-w-0">
          ...
          >
            Mark any object and erase it cleanly.
          </p>
        </div>
        <Link
          to="/studio/image/circle-remove"
          search={{ mode: "remove", from: "home" }}
  ...
  return (
    ...
    >
      ...
      <div className="flex flex-1 flex-col gap-2 p-3.5 sm:p-4">
        ...
        <Link
          to="/studio/image/circle-remove"
          search={{ mode: "add", from: "home", assetId: "animal_deer" }}
          className="mt-1 inline-flex w-full items-center justify-center gap-1.5 rounded-2xl bg-[#7B6FE0] px-3 py-2.5 text-[13px] font-semibold text-white shadow-sm shadow-[rgba(123,111,224,0.3)] transition active:scale-[0.98]"
        >
          <Sparkles className="h-3.5 w-3.5" />
          Try Now
