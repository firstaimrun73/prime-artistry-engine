...
export function EffectStudioPage({
      ...
      <div className="flex min-h-[100dvh] flex-col bg-[#FFFBF7] text-[#161412]">
        ...
        <main className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center px-4 py-10 min-h-0">
          <button type="button" onClick={() => inputRef.current?.click()} disabled={busy}
            ...
            <span className="text-sm text-[#6F6862]">JPG, PNG or WebP</span>
          </button>
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-[#FFFBF7] text-[#161412]">
      ...
      <section className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden px-3 py-2" style={{ maxHeight: "min(58dvh, 100%)" }}>
            ...
            {editorTab === "filter" ? (
              <div className="space-y-3 px-3">
                <div className="flex gap-2 overflow-x-auto px-1 scrollbar-none">
                  <button type="button" onClick={() => setCategory("all")} className={cn("h-8 shrink-0 rounded-full px-3 text-xs font-semibold", category === "all" ? "bg-[#FF5A1F] text-white" : "bg-[#F5F0EB] text-[#6F6862]")}>All</button>
                  {sortedCategories.map((c) => (
                    <button key={c} type="button" onClick={() => setCategory(c)} className={cn("h-8 shrink-0 rounded-full px-3 text-xs font-semibold", category === c ? "bg-[#FF5A1F] text-white" : "bg-[#F5F0EB] text-[#6F6862]")}>{c}</button>
                  ))}
                </div>
              ...
              <div className="space-y-3 px-3">
                ...
                <div className="flex items-center gap-3 px-1">
                  ...
                  <span className="w-10 text-right text-xs font-semibold tabular-nums text-[#161412]">{adj[adjKey]}</span>
                </div>
                {(adjKey === "color") && (
