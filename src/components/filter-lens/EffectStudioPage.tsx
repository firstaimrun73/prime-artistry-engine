            ...
            <div className="mx-auto flex max-w-6xl flex-col gap-2">
              ...
              <div className="flex gap-2">
                {!selected.isFree && !isUnlocked(selected.id, selected.isFree) ? (
                  <button type="button" onClick={() => void onUnlock()} disabled={busy} className="flex-1 rounded-xl bg-[#7B6FE0] py-2.5 text-sm font-semibold text-white">
                    Unlock · {selected.unlockCost} credits
                  </button>
                ) : (
                  <button type="button" onClick={() => void onApply()} disabled={busy || !file} className="flex-1 rounded-xl bg-[#7B6FE0] py-2.5 text-sm font-semibold text-white disabled:opacity-40">
                    {busy ? "Working…" : "Apply"}
                  </button>
                )}
                {resultUrl ? (
                  <button
                    ...
                    className={cn("grid h-10 w-10 place-items-center rounded-xl border", isDark ? "border-white/10" : "border-black/8")}
                    aria-label="Download"
                  >
