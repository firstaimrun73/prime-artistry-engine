      ...
      {phase === "result" ? (
        <div className="shrink-0 border-t border-[#E8E0D8] bg-white/95 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-md">
          <div className="mb-3 flex items-center justify-center gap-8">
            <button type="button" onClick={onRestart} className="grid h-11 w-11 place-items-center rounded-xl" aria-label="Restart">
              <RotateCcw className="h-5 w-5" />
            </button>
            <button type="button" onPointerDown={() => setComparing(true)} onPointerUp={() => setComparing(false)} onPointerLeave={() => setComparing(false)} className={cn("grid h-11 w-11 place-items-center rounded-xl", comparing && "bg-[#FFE6DA] text-[#FF5A1F]")} aria-label="Compare">
              <Columns2 className="h-5 w-5" />
            </button>
            <button type="button" onClick={onDownload} className="grid h-11 w-11 place-items-center rounded-xl bg-[#FFE8DA]" aria-label="Download">
          ...
          <button type="button" onClick={onDownload} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#FF5A1F] py-3 text-sm font-bold text-white">
          ...
          <div className="shrink-0 border-t border-[#E8E0D8]/60 bg-white/95 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-md">
            <div className="mb-2 flex items-center gap-6 px-4">
              <button type="button" onClick={() => setEditorTab("filter")} className={cn("relative pb-2 text-[15px] font-semibold uppercase tracking-wide", editorTab === "filter" ? "text-[#FF5A1F]" : "text-[#6F6862]")}>
                Filter
                {editorTab === "filter" && <span className="absolute bottom-0 left-0 right-0 h-0.5 rounded bg-[#FF5A1F]" />}
              </button>
              <button type="button" onClick={() => setEditorTab("adjust")} className={cn("relative pb-2 text-[15px] font-semibold uppercase tracking-wide", editorTab === "adjust" ? "text-[#FF5A1F]" : "text-[#6F6862]")}>
                Adjust
                {editorTab === "adjust" && <span className="absolute bottom-0 left-0 right-0 h-0.5 rounded bg-[#FF5A1F]" />}
              </button>
              <button type="button" onClick={() => void onApply()} className="ml-auto grid h-10 w-10 place-items-center text-[#FF5A1F]" aria-label="Apply">
            ...
            {editorTab === "filter" ? (
              <div className="space-y-3 px-3">
                <div className="flex gap-2 overflow-x-auto px-1 scrollbar-none">
                  <button type="button" onClick={() => setCategory("all")} className={cn("h-8 shrink-0 rounded-full px-3.5 text-[13px] font-medium", category === "all" ? "bg-[#FF5A1F] text-white" : "border border-[#E8E0D8] bg-white")}>
                    All
                  </button>
                  {sortedCategories.map((c) => (
                    <button key={c} type="button" onClick={() => setCategory(c)} className={cn("h-8 shrink-0 rounded-full px-3.5 text-[13px] font-medium", category === c ? "bg-[#FF5A1F] text-white" : "border border-[#E8E0D8] bg-white")}>
                      {c}
                    </button>
                  ))}
                </div>
                <p className="px-1 text-xs text-[#6F6862]">{filtered.length} looks · live preview of this photo</p>
                    ...
                    return (
                      <button key={item.id} type="button" onClick={() => selectFilter(item)} className="w-[78px] shrink-0 text-left">
                        <div className={cn("relative aspect-[1/1.05] overflow-hidden rounded-lg bg-[#141210]", isSel && "outline outline-2 outline-offset-1 outline-[#FF5A1F]")}>
                          ...
                          {isSel && (
                            <span className="absolute right-1 top-1 grid h-[18px] w-[18px] place-items-center rounded-full bg-[#FF5A1F] text-[11px] font-bold text-white">✓</span>
                          )}
                          <span className="absolute inset-x-0 bottom-0 bg-black/70 px-1 py-0.5 text-center text-[9px] font-semibold uppercase tracking-wide text-white truncate">
                            {item.name}
                          </span>
                        </div>
                      </button>
                  ...
                  <div className="mx-1 rounded-2xl border border-[#FF5A1F]/15 bg-gradient-to-b from-[#FFF5EE] to-[#FFEEE4] p-3">
                    ...
                    <div className="mb-3 flex flex-wrap justify-center gap-2.5">
                      {COLOR_SWATCHES.map((s) => (
                        <button key={s.id} type="button" onClick={() => { bumpToEdit(); setColorId(s.id); }} className={cn("h-7 w-7 rounded-full border-2", colorId === s.id ? "border-white shadow-[0_0_0_2px_#FF5A1F] scale-110" : "border-transparent")} style={{ background: s.hex }} aria-label={s.id} />
                      ))}
                    </div>
                  </div>
                )}
