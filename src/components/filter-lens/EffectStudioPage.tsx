...
export function EffectStudioPage({
  ...
  const onDownload = () => {
    const url = resultWmUrl || resultUrl || previewUrl;
    if (!url) return;
  ...
  const displayResultUrl = resultWmUrl || resultUrl;
            ...
            {editorTab === "filter" ? (
                    ...
                    return (
                      ...
                      >
                        {thumbMap[item.id] ? (
                          ...
                          <div className="flex h-full w-full items-center justify-center bg-[#F5F0EA]">
                            {thumbsBusy ? (
                              <Loader2 className="h-4 w-4 animate-spin text-[#FF5A1F]/60" />
                            ) : (
                              <span className="text-[10px] text-[#6F6862]">{item.name.slice(0, 6)}</span>
                            )}
                          </div>
                        )}
                        {active && (
              ...
              <div className="mx-auto max-w-lg space-y-3 px-3">
                <div className="flex items-center justify-between px-1">
                  <span className="text-[11px] font-semibold uppercase tracking-wide text-[#6F6862]">
                    Adjust
                  </span>
                  <button
                    type="button"
                    onClick={resetAdjust}
                    className="text-[11px] font-semibold text-[#FF5A1F]"
                  >
                    Reset
                  </button>
                </div>
                  ...
                  {ADJUST_META.map((m) => {
                    ...
                    return (
                      <button
                        ...
                        className={cn(
                          "flex min-w-[56px] flex-col items-center gap-1 rounded-xl px-2 py-2 text-[10px] font-semibold transition",
                          active
                            ? "bg-[#FFF1E6] text-[#FF5A1F]"
                            : "text-[#6F6862]",
                        )}
                      >
