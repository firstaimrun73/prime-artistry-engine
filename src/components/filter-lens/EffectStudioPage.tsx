...
export function EffectStudioPage({
  ...
  const selectFilter = (item: CatalogItem) => {
    ...
    // Filter switch must not inherit previous filter's adjustments
    setAdj({ ...DEFAULT_ADJ });
        ...
        <main className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center px-4 py-10 min-h-0">
          <button type="button" onClick={() => inputRef.current?.click()} disabled={busy}
            ...
            <span className="text-center">
              <span className="block text-base font-bold text-[#161412]">Choose a photo</span>
              <span className="mt-1 block text-sm text-[#6F6862]">JPG, PNG or WebP</span>
            </span>
          </button>
              ...
              <div className="space-y-3 px-3">
                ...
                <div className="flex items-center gap-3 px-1">
                  ...
                  <OrangeSlider
                    ...
                    ariaLabel="Filter intensity"
                  />
                  <span className="w-8 text-right text-xs font-semibold tabular-nums text-[#161412]">{intensity}</span>
                </div>
              ...
              <div className="space-y-3 px-4">
                <div className="flex gap-2 overflow-x-auto scrollbar-none">
                  {ADJUST_META.map((m) => {
                    const Icon = m.icon;
                    return (
                      <button key={m.key} type="button" onClick={() => setAdjKey(m.key)} className={cn("flex shrink-0 flex-col items-center gap-1 rounded-xl px-2.5 py-1.5", adjKey === m.key ? "bg-[#FFE8DA] text-[#FF5A1F]" : "text-[#6F6862]")}>
