...
export function EffectStudioPage({
  ...
  if (!hasPhoto) {
    return (
      <div className="flex min-h-[100dvh] flex-col bg-[#FFFBF7] text-[#161412] dark:bg-[#121110] dark:text-[#F5F0EB]">
        <header className="flex items-center gap-3 border-b border-[#E8E0D8] dark:border-[#2A2724] px-3 py-3 pt-[max(0.75rem,env(safe-area-inset-top))]">
          <Link to="/" className="grid h-9 w-9 place-items-center rounded-full border border-[#E8E0D8] dark:border-[#2A2724] bg-white dark:bg-[#1A1816]" aria-label="Back"><ArrowLeft className="h-4 w-4" /></Link>
  ...
  return (
    <div className="flex h-[100dvh] flex-col overflow-hidden bg-[#FFFBF7] text-[#161412] dark:bg-[#121110] dark:text-[#F5F0EB]">
      <header className="flex shrink-0 items-center gap-3 border-b border-[#E8E0D8] dark:border-[#2A2724] bg-white dark:bg-[#1A1816] px-3 py-2.5 pt-[max(0.75rem,env(safe-area-inset-top))]">
        <Link to="/" className="grid h-9 w-9 place-items-center rounded-full border border-[#E8E0D8] dark:border-[#2A2724] bg-white dark:bg-[#1A1816]" aria-label="Back"><ArrowLeft className="h-4 w-4" /></Link>
        <div className="min-w-0 flex-1">
          <p className="text-[10px] font-semibold tracking-[0.16em] text-[#FF5A1F] uppercase">Motio2edit</p>
          <FiltersTitle className="truncate text-base font-bold" />
        </div>
        <button type="button" onClick={() => inputRef.current?.click()} className="grid h-9 w-9 place-items-center rounded-xl border border-[#E8E0D8] dark:border-[#2A2724] bg-white dark:bg-[#1A1816]" aria-label="Replace photo"><ImagePlus className="h-4 w-4" /></button>
        <input ref={inputRef} type="file" accept="image/*" className="hidden" onChange={(e) => void onPick(e.target.files?.[0] ?? null)} />
      </header>
      ...
      <section className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden px-3 py-2">
        {comparing && sourceUrl && displayResultUrl ? (
          ...
          <img src={displayResultUrl || processedUrl || sourceUrl!} alt="Result" className="h-auto max-h-full w-auto max-w-full rounded-xl object-contain" draggable={false} />
        ) : (
      ...
      {phase === "result" ? (
        <div className="shrink-0 border-t border-[#E8E0D8] dark:border-[#2A2724] bg-white/95 dark:bg-[#1A1816]/95 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-md">
          ...
          <div className="mb-3 flex flex-col items-center gap-1">
            <button
              ...
              className={cn(
                ...
                freeUser
                  ? "cursor-not-allowed border-[#E8E0D8] dark:border-[#2A2724] bg-[#F5F0EB] dark:bg-[#2A2724] text-[#6F6862] dark:text-[#A8A29A] opacity-90"
                  : showWm
                    ? "border-[#FF5A1F]/40 bg-[#FFE8DA] text-[#FF5A1F]"
                    : "border-[#E8E0D8] dark:border-[#2A2724] bg-white dark:bg-[#1A1816] text-[#161412]",
              )}
              aria-pressed={showWm}
              aria-label={freeUser ? "Watermark locked on free plan" : "Toggle watermark"}
            >
              <span className={cn("relative inline-flex h-4 w-7 items-center rounded-full transition", showWm ? "bg-[#FF5A1F]" : "bg-[#D4CDC4]")} aria-hidden>
                <span className={cn("absolute h-3 w-3 rounded-full bg-white shadow transition", showWm ? "right-0.5" : "left-0.5")} />
              </span>
              {freeUser ? <span aria-hidden>🔒</span> : null}
              Watermark
            </button>
            {freeUser ? (
              <p className="text-center text-[10px] text-[#6F6862] dark:text-[#A8A29A]">Free plan includes Motio2edit watermark</p>
            ) : null}
          </div>
          <div className="flex gap-2">
            ...
            <button type="button" onClick={() => void onShare()} className="flex flex-1 items-center justify-center gap-2 rounded-2xl border border-[#E8E0D8] dark:border-[#2A2724] bg-white dark:bg-[#1A1816] py-3 text-sm font-bold text-[#161412]"><Share2 className="h-4 w-4" /> Share</button>
          </div>
        </div>
      ) : (
        <>
          <div className="flex shrink-0 items-center justify-center gap-10 border-t border-[#E8E0D8]/80 dark:border-[#2A2724] bg-white/90 dark:bg-[#1A1816]/90 px-4 py-1.5 backdrop-blur-sm">
            <button type="button" onClick={onUndo} disabled={!canUndo} className="grid h-10 w-10 place-items-center rounded-xl disabled:opacity-30" aria-label="Undo"><Undo2 className="h-5 w-5" /></button>
            <button type="button" onClick={onRedo} disabled={!canRedo} className="grid h-10 w-10 place-items-center rounded-xl disabled:opacity-30" aria-label="Redo"><Redo2 className="h-5 w-5" /></button>
          </div>
          <div className="shrink-0 border-t border-[#E8E0D8]/60 dark:border-[#2A2724] bg-white/95 dark:bg-[#1A1816]/95 pb-[max(0.75rem,env(safe-area-inset-bottom))] pt-2 backdrop-blur-md">
            <div className="mb-2 flex items-center gap-6 px-4">
              <button type="button" onClick={() => setEditorTab("filter")} className={cn("relative pb-2 text-[15px] font-semibold uppercase tracking-wide", editorTab === "filter" ? "text-[#FF5A1F]" : "text-[#6F6862] dark:text-[#A8A29A]")}>
                Filter{editorTab === "filter" && <span className="absolute bottom-0 left-0 right-0 h-0.5 rounded bg-[#FF5A1F]" />}
              </button>
              <button type="button" onClick={() => setEditorTab("adjust")} className={cn("relative pb-2 text-[15px] font-semibold uppercase tracking-wide", editorTab === "adjust" ? "text-[#FF5A1F]" : "text-[#6F6862] dark:text-[#A8A29A]")}>
            ...
            {editorTab === "filter" ? (
              <div className="space-y-3 px-3">
                    ...
                    return (
                      <button key={item.id} type="button" onClick={() => selectFilter(item)} className="w-[78px] shrink-0 text-left">
                        <div className={cn("relative aspect-[1/1.05] overflow-hidden rounded-lg bg-[#141210]", isSel && isUnlocked(item) && "outline outline-2 outline-offset-1 outline-[#FF5A1F]")}>
                          ...
                          {isSel && isUnlocked(item) && <span className="absolute right-1 top-1 z-10 grid h-[18px] w-[18px] place-items-center rounded-full bg-[#FF5A1F] text-[11px] font-bold text-white">✓</span>}
                          ...
                          {!isUnlocked(item) && (
                            <span className="absolute inset-0 z-[5] grid place-items-center bg-black/45" aria-label="Locked">
                              <span className="grid h-8 w-8 place-items-center rounded-full bg-black/70 text-white shadow-md">
                          ...
                          <span className="absolute inset-x-0 bottom-0 z-10 bg-black/70 px-1 py-0.5 text-center text-[9px] font-semibold uppercase tracking-wide text-white truncate">{item.name}</span>
                        </div>
                      </button>
                    );
                  })}
                ...
                <div className="flex items-center gap-3 px-1">
                  <span className="w-16 shrink-0 text-[11px] font-semibold text-[#6F6862] dark:text-[#A8A29A]">Intensity</span>
                ...
                <button type="button" disabled={busy || (selected ? !isUnlocked(selected) : true)} onClick={() => void onApply()} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#FF5A1F] py-3 text-sm font-bold text-white disabled:opacity-40">{busy ? "Applying…" : "Apply filter"}</button>
              </div>
            ) : (
              <div className="space-y-3 px-3">
                <div className="px-1">
                ...
                <div className="flex gap-1 overflow-x-auto px-1 scrollbar-none">
                  {ADJUST_META.map((m) => {
                    const Icon = m.icon;
                    return (
                      <button key={m.key} type="button" onClick={() => setAdjKey(m.key)} className={cn("flex min-w-[64px] shrink-0 flex-col items-center gap-1 px-2 py-1.5 text-[10px] font-semibold uppercase tracking-wide", adjKey === m.key ? "text-[#FF5A1F]" : "text-[#6F6862] dark:text-[#A8A29A]")}>
                ...
                {adjKey === "color" && (
                  <div className="mx-1 rounded-2xl border border-[#FF5A1F]/15 bg-gradient-to-b from-[#FFF5EE] to-[#FFEEE4] p-3">
                    <div className="mb-3 flex justify-center gap-6">
                      <button type="button" onClick={() => setColorTarget("highlight")} className={cn("text-xs font-semibold uppercase tracking-wide", colorTarget === "highlight" ? "text-[#FF5A1F]" : "text-[#6F6862] dark:text-[#A8A29A]")}>Highlight</button>
                      <button type="button" onClick={() => setColorTarget("shadow")} className={cn("text-xs font-semibold uppercase tracking-wide", colorTarget === "shadow" ? "text-[#FF5A1F]" : "text-[#6F6862] dark:text-[#A8A29A]")}>Shadow</button>
                    </div>
                    <div className="mb-3 flex flex-wrap justify-center gap-2.5">
                      {COLOR_SWATCHES.map((s) => {
                        ...
                        return (
                          <button key={s.id} type="button" onClick={() => { bumpToEdit(); if (colorTarget === "highlight") { setHighlightColorId(s.id); pushHistory({ selectedId, intensity, adj: { ...adj }, highlightColorId: s.id, shadowColorId }); } else { setShadowColorId(s.id); pushHistory({ selectedId, intensity, adj: { ...adj }, highlightColorId, shadowColorId: s.id }); } }} className={cn("h-7 w-7 rounded-full border-2", activeId === s.id ? "border-white shadow-[0_0_0_2px_#FF5A1F] scale-110" : "border-transparent")} style={{ background: `rgb(${s.rgb[0]},${s.rgb[1]},${s.rgb[2]})` }} aria-label={s.id} />
                        );
                      })}
                    </div>
                  </div>
                )}
                <button type="button" onClick={resetAdjust} className="text-center text-xs text-[#6F6862] dark:text-[#A8A29A] underline">Reset adjustments</button>
                <button type="button" disabled={busy || (selected ? !isUnlocked(selected) : true)} onClick={() => void onApply()} className="flex w-full items-center justify-center gap-2 rounded-2xl bg-[#FF5A1F] py-3 text-sm font-bold text-white disabled:opacity-40">{busy ? "Applying…" : "Apply filter"}</button>
              </div>
            )}
          </div>
