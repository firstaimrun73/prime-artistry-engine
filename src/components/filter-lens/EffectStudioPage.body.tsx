...
export function EffectStudioPage({
      ...
      <div className="flex min-h-[100dvh] flex-col bg-[#FFFBF7] text-[#161412]">
        ...
        <main className="mx-auto flex w-full max-w-md flex-1 flex-col items-center justify-center px-4 py-10 min-h-0">
          ...
          <button type="button" onClick={() => inputRef.current?.click()} disabled={busy}
            ...
            {busy ? <Loader2 className="h-8 w-8 animate-spin text-[#FF5A1F]" /> : <ImagePlus className="h-8 w-8 text-[#FF5A1F]" />}
            <span className="text-base font-bold text-[#161412]">Drop image or tap to upload</span>
  ...
  return (
    <div className="flex min-h-[100dvh] flex-col bg-[#FFFBF7] text-[#161412]">
      <header className="flex shrink-0 items-center gap-3 border-b border-[#E8E0D8] bg-white px-3 py-2.5 pt-[max(0.75rem,env(safe-area-inset-top))]">
        <Link to="/" className="grid h-9 w-9 place-items-center rounded-full border border-[#E8E0D8] bg-white" aria-label="Back"><ArrowLeft className="h-4 w-4" /></Link>
        <div className="min-w-0 flex-1">
          <p className="text-[10px] font-semibold tracking-[0.16em] text-[#FF5A1F] uppercase">Motio2edit</p>
          <FiltersTitle className="truncate text-lg font-bold tracking-tight" />
        </div>
        <button type="button" onClick={() => inputRef.current?.click()} className="grid h-9 w-9 place-items-center rounded-xl border border-[#E8E0D8] bg-white" aria-label="Replace photo"><ImagePlus className="h-4 w-4" /></button>
        <input ref={inputRef} type="file" accept="image/*" className="hidden" onChange={(e) => void onPick(e.target.files?.[0] ?? null)} />
      </header>
      ...
      <div className="relative flex min-h-0 flex-1 flex-col items-center justify-center overflow-hidden px-3 py-2">
        ...
        {busy && (
          <div className="absolute inset-0 grid place-items-center bg-[#FFFBF7]/70 text-sm font-medium">Working…</div>
        )}
      </div>
        ...
        <div className="shrink-0 border-t border-[#E8E0D8] bg-white/95 px-4 pb-[max(1rem,env(safe-area-inset-bottom))] pt-3 backdrop-blur-md">
          ...
          <div className="mb-3 flex flex-col items-center gap-1">
            ...
            >
              ...
              {freeUser ? (
                <svg className="h-3 w-3 shrink-0 text-[#6F6862]" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" aria-hidden>
            ...
            {editorTab === "filter" ? (
              <div className="space-y-3 px-3">
                <div className="flex gap-2 overflow-x-auto px-1 scrollbar-none">
                  <button type="button" onClick={() => setCategory("all")} className={cn("shrink-0 rounded-full border px-3.5 py-1.5 text-[13px] font-medium", category === "all" ? "border-[#FF5A1F] bg-[#FF5A1F] text-white" : "border-[#E8E0D8] bg-white text-[#161412]")}>All</button>
                  {sortedCategories.map((c) => (
                    <button key={c} type="button" onClick={() => setCategory(c)} className={cn("shrink-0 rounded-full border px-3.5 py-1.5 text-[13px] font-medium", category === c ? "border-[#FF5A1F] bg-[#FF5A1F] text-white" : "border-[#E8E0D8] bg-white text-[#161412]")}>{c}</button>
                  ))}
                </div>
                    ...
                    return (
                      <button key={item.id} type="button" onClick={() => selectFilter(item)} className="w-[78px] shrink-0 text-left">
                        <div className={cn("relative aspect-[1/1.05] overflow-hidden rounded-lg bg-[#141210]", isSel && isUnlocked(item) && "outline outline-2 outline-offset-1 outline-[#FF5A1F]")}>
                          ...
                          {!isUnlocked(item) ? (
                            <span className="absolute right-1 top-1 grid h-[18px] w-[18px] place-items-center rounded-full bg-black/65 text-white" aria-label="Locked">
                            ...
                            item.badge === "ai+" ? (
                              <span className="absolute left-1 top-1 inline-flex items-center rounded bg-[#FF5A1F] px-1 py-0.5 text-[8px] font-bold uppercase tracking-wide text-white">AI+</span>
                            ) : (
                ...
                {adjKey === "color" && (
                  <div className="mx-1 rounded-2xl border border-[#FF5A1F]/15 bg-gradient-to-b from-[#FFF5EE] to-[#FFEEE4] p-3">
                    <div className="mb-3 flex justify-center gap-6">
                      <button type="button" onClick={() => setColorTarget("highlight")} className={cn("text-xs font-semibold uppercase tracking-wide", colorTarget === "highlight" ? "text-[#FF5A1F]" : "text-[#6F6862]")}>Highlight</button>
                      <button type="button" onClick={() => setColorTarget("shadow")} className={cn("text-xs font-semibold uppercase tracking-wide", colorTarget === "shadow" ? "text-[#FF5A1F]" : "text-[#6F6862]")}>Shadow</button>
                    </div>
