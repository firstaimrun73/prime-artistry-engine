 ...
 * Restored. Back → home (replace). How-it-works via AutoEditHowItWorks (curved dotted lines).
 */
...
import { AutoEditHowItWorks } from "@/components/auto-edit/AutoEditHowItWorks";
  ...
  const run = async () => {
    ...
    try {
      ...
      const result = await runEdit({
        data: { imageUrl: uploaded, imageQuality: quality },
      });
      setStep(3);
      setProgress(85);
      const outUrl =
    ...
    return (
      <div className="min-h-screen bg-gradient-to-b from-violet-50 via-background to-background dark:from-violet-950/40">
        <main className="mx-auto max-w-md px-4 py-16 text-center">
          ...
          <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-violet-600 dark:text-violet-300">Motio2edit</p>
          <h1 className="mt-1 text-2xl font-extrabold tracking-tight">{AUTO_EDIT_PRODUCT_NAME}</h1>
          <p className="mt-2 text-sm text-muted-foreground">Sign in to enhance one photo.</p>
          <Link to="/auth" search={{ redirect: "/studio/image/auto-edit" }} className="mt-6 inline-flex rounded-full bg-primary px-5 py-2.5 text-sm font-semibold text-primary-foreground">
            Sign in
          </Link>
        </main>
      </div>
    ...
    <div className="min-h-screen bg-gradient-to-b from-violet-50/90 via-background to-cyan-50/30 dark:from-violet-950/40 dark:via-background dark:to-background">
      <main className="mx-auto max-w-xl px-4 py-6 sm:py-10">
        <div className="mb-5 flex items-center gap-3">
          <button type="button" onClick={goHome} className="grid h-9 w-9 shrink-0 place-items-center rounded-full border border-white/40 bg-white/50 shadow-sm backdrop-blur-md dark:border-white/10 dark:bg-white/5" aria-label="Back to home">
          ...
          <div className="min-w-0 flex-1">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="relative inline-flex h-6 w-6 items-center justify-center" title="Maluto AI is live" aria-label="Live">
          ...
          {PIPELINE.map((label, i) => {
            ...
            return (
              <div key={label} className="flex flex-1 flex-col items-center gap-1">
                <span className={cn("grid h-6 w-6 place-items-center rounded-full text-[10px] font-bold transition", done && "bg-violet-600 text-white", active && "animate-pulse bg-cyan-500 text-white", !done && !active && "bg-black/5 text-muted-foreground dark:bg-white/10")}>
        ...
        <div
          ...
          onDrop={onDrop}
          className={cn("relative overflow-hidden rounded-3xl border border-white/60 bg-white/45 p-5 shadow-lg backdrop-blur-xl transition dark:border-white/10 dark:bg-white/5", dragOver && "border-violet-400 ring-2 ring-violet-400/30")}
        >
          {!preview ? (
            <button type="button" onClick={() => inputRef.current?.click()} className="flex w-full flex-col items-center gap-3 py-14 text-center">
            ...
            <div className="space-y-4">
              <div className="relative overflow-hidden rounded-2xl bg-black/5">
                {output && preview ? (
                  ...
                  <img src={preview} alt="Preview" className="mx-auto max-h-[400px] w-auto object-contain" />
                )}
                {!output && (
                  <button type="button" onClick={clear} className="absolute right-2 top-2 grid h-8 w-8 place-items-center rounded-full border border-white/30 bg-black/45 text-white backdrop-blur" aria-label="Clear">
                    <X className="h-4 w-4" />
                  </button>
                )}
              </div>
              ...
              <div className="flex flex-wrap items-center gap-2">
                {qualityOptions.map((opt) => (
                  <button key={opt.id} type="button" onClick={() => setQuality(opt.id)} className={cn("rounded-full border px-3 py-1 text-xs font-semibold backdrop-blur", quality === opt.id ? "border-violet-500 bg-violet-500/15 text-violet-700 dark:text-violet-200" : "border-white/40 bg-white/30 text-muted-foreground dark:border-white/10 dark:bg-white/5")}>
                    {opt.label}
                  </button>
                ))}
                {!isAdmin && (
                  <div className="relative ml-auto">
                    <button type="button" onClick={revealCost} className="grid h-8 w-8 place-items-center rounded-full border border-white/40 bg-white/40 text-violet-600 backdrop-blur dark:border-white/10 dark:bg-white/5 dark:text-violet-300" aria-label="Show credit costs">
                    ...
                    {showCost && (
                      <div className="absolute right-0 top-full z-30 mt-2 w-44 overflow-hidden rounded-2xl border border-violet-300/40 bg-white/95 p-2 shadow-xl backdrop-blur-xl dark:border-violet-500/30 dark:bg-violet-950/95">
                        <p className="mb-1.5 px-1 text-[10px] font-bold uppercase tracking-wide text-violet-600 dark:text-violet-300">Credit costs</p>
                        <div className="space-y-0.5">
                          {CREDIT_TABLE.map((row) => (
                            <div key={row.label} className={cn("flex items-center justify-between rounded-lg px-2 py-1 text-[11px]", row.credits === creditCost ? "bg-violet-500/15 font-semibold text-violet-700 dark:text-violet-200" : "text-muted-foreground")}>
                              <span>{row.label}</span>
                              <span>{row.credits}</span>
                            </div>
                          ))}
                        </div>
                        <p className="mt-1.5 border-t border-violet-200/50 px-1 pt-1.5 text-[10px] text-muted-foreground dark:border-violet-500/20">Selected: <strong>{creditCost} credits</strong></p>
                      </div>
                    )}
                  </div>
                )}
              ...
              {busy && (
                <div className="space-y-2">
                  ...
                  <p className="text-center text-xs text-muted-foreground">{PIPELINE[Math.min(step, PIPELINE.length - 1)]}… {progress}%</p>
                </div>
              )}

              <div className="space-y-2">
                <Button type="button" onClick={() => void run()} disabled={busy || !file} className="h-12 w-full rounded-full bg-gradient-to-r from-violet-600 to-cyan-500 text-base font-semibold text-white shadow-md">
                  {busy ? (<><Loader2 className="mr-2 h-4 w-4 animate-spin" /> Working</>) : (<><Sparkles className="mr-2 h-4 w-4" /> Enhance</>)}
                </Button>
                {output && (
                  <div className="flex flex-wrap gap-2">
                    <Button type="button" variant="outline" className="flex-1 rounded-full border-white/50 bg-white/40 backdrop-blur dark:bg-white/5" onClick={() => setViewerOpen(true)}>
                      <ImageIcon className="mr-2 h-4 w-4" /> View
                    </Button>
                    <Button type="button" variant="outline" className="flex-1 rounded-full border-white/50 bg-white/40 backdrop-blur dark:bg-white/5" disabled={dlBusy} onClick={() => void downloadResult()}>
        ...
        <section className="mt-10 space-y-2">
          ...
          <div className="space-y-2">
            {FAQ_ITEMS.map((item, i) => {
              ...
              return (
                <div key={item.q} className="overflow-hidden rounded-2xl border border-white/50 bg-white/40 backdrop-blur-md dark:border-white/10 dark:bg-white/5">
                  <button type="button" onClick={() => setOpenFaq(open ? null : i)} className="flex w-full items-center justify-between gap-2 px-4 py-3 text-left">
                    <span className="text-sm font-medium">{item.q}</span>
                    <ChevronDown className={cn("h-4 w-4 shrink-0 text-muted-foreground transition", open && "rotate-180")} />
                  </button>
                  {open && (
                    <p className="border-t border-white/30 px-4 py-3 text-xs leading-relaxed text-muted-foreground dark:border-white/10">{item.a}</p>
                  )}
                </div>
              );
            })}
