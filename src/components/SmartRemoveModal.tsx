...
export function SmartRemoveModal({ open, imageUrl, onCancel, onApply }: Props) {
  ...
  // Fit image to natural aspect ratio — never force fixed box.
  // Viewport is flex-1 ABOVE the bottom drawer — do not subtract drawer height again.
  ...
  const pointFromClient = (clientX: number, clientY: number) => {
    ...
    if (!n || rect.width < 1 || rect.height < 1) return { x: 0, y: 0 };
  ...
  const fillCircleRegion = (a: { x: number; y: number }, b: { x: number; y: number }) => {
    const c = maskCanvasRef.current;
    if (!c) return;
    const ctx = c.getContext("2d");
    if (!ctx) return;
    const ms = maskScaleRef.current;
    const cx = ((a.x + b.x) / 2) * ms;
    const cy = ((a.y + b.y) / 2) * ms;
    const rx = (Math.abs(b.x - a.x) / 2) * ms;
    const ry = (Math.abs(b.y - a.y) / 2) * ms;
    if (rx < 1 && ry < 1) return;
    ctx.save();
    ctx.globalCompositeOperation = "source-over";
  ...
  const continueStroke = (clientX: number, clientY: number) => {
    ...
    if (tool === "circle" && circleStartRef.current) {
      ...
      if (view && n) {
        ...
        if (vctx) {
          const a = circleStartRef.current;
          const cx = ((a.x + p.x) / 2) * displayScale;
          const cy = ((a.y + p.y) / 2) * displayScale;
          const rx = (Math.abs(p.x - a.x) / 2) * displayScale;
          const ry = (Math.abs(p.y - a.y) / 2) * displayScale;
          vctx.save();
          vctx.strokeStyle = "rgba(239, 68, 68, 0.95)";
          vctx.lineWidth = 2;
          vctx.setLineDash([6, 4]);
          vctx.beginPath();
          vctx.ellipse(cx, cy, Math.max(rx, 2), Math.max(ry, 2), 0, 0, Math.PI * 2);
          vctx.stroke();
          vctx.restore();
        }
      }
  ...
  const endStroke = () => {
    ...
    if (tool === "circle" && circleStartRef.current && lastPtRef.current) {
      fillCircleRegion(circleStartRef.current, lastPtRef.current);
      circleStartRef.current = null;
      requestPaint();
      pushHistory();
  ...
  return (
    <div className="fixed inset-0 z-50 flex flex-col bg-black/85" role="dialog" aria-modal="true">
        ...
        <div className="flex items-center justify-between border-b border-white/10 bg-background/95 px-4 py-3">
          <div>
            ...
            <p className="text-xs text-muted-foreground">
              Mark what to remove. Space = pan · C circle · B brush · E erase · Ctrl+Z undo
            </p>
          </div>
      ...
      {isMobile ? (
        <div ref={bottomControlsRef} className="space-y-1.5 border-t border-white/10 bg-background/95 px-2.5 pt-2 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
          <div className="flex items-center justify-between gap-2">
            <div className="inline-flex overflow-hidden rounded-md border border-border">
              <button type="button" onClick={() => setTool("circle")} title="Circle region" className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 ${tool === "circle" ? "bg-primary text-primary-foreground" : "hover:bg-secondary"}`}>
                <Circle className="h-3.5 w-3.5" />
              </button>
              <button type="button" onClick={() => setTool("brush")} title="Brush mark" className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 ${tool === "brush" ? "bg-primary text-primary-foreground" : "hover:bg-secondary"}`}>
                <Brush className="h-3.5 w-3.5" />
              </button>
              <button type="button" onClick={() => setTool("erase")} title="Erase mark" className={`inline-flex items-center gap-1.5 px-2.5 py-1.5 ${tool === "erase" ? "bg-primary text-primary-foreground" : "hover:bg-secondary"}`}>
                <Eraser className="h-3.5 w-3.5" />
              </button>
            </div>
            <div className="flex items-center gap-1.5">
              <button type="button" onClick={undo} disabled={!canUndo} className="inline-flex items-center rounded-md border border-border px-2.5 py-1.5 hover:bg-secondary disabled:opacity-40">
                <Undo2 className="h-3.5 w-3.5" />
              </button>
              <button type="button" onClick={redo} disabled={!canRedo} className="inline-flex items-center rounded-md border border-border px-2.5 py-1.5 hover:bg-secondary disabled:opacity-40">
                <Redo2 className="h-3.5 w-3.5" />
              </button>
              <button type="button" onClick={clearMask} disabled={!hasMark} className="inline-flex items-center rounded-md border border-border px-2.5 py-1.5 hover:bg-secondary disabled:opacity-40">
          ...
          <div className="flex items-center gap-2">
            ...
            <span className="w-10 text-right tabular-nums text-xs">{brush}</span>
          </div>

          <div className="flex items-center justify-end gap-2">
            <Button variant="ghost" size="sm" onClick={onCancel} disabled={applying}>Cancel</Button>
            <Button
              onClick={apply}
              disabled={!hasMark || applying}
              className="h-10 w-40 bg-orange-500 text-sm font-semibold text-white shadow-lg hover:bg-orange-600 disabled:opacity-60"
            >
              {applying ? (
                <>
