...
export function LensEditor({ initialLensId }: { initialLensId?: string }) {
  ...
  // Single-surface: always draw camera → canvas only
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const sourceW = Math.round(video.videoWidth * scale);
      const sourceH = Math.round(video.videoHeight * scale);
      if (tmp.width !== sourceW || tmp.height !== sourceH) {
        tmp.width = sourceW;
        tmp.height = sourceH;
      }
      // Keep the processing canvas at the source aspect ratio.
      ...
      tctx.clearRect(0, 0, sourceW, sourceH);
      if (facingMode === "user") {
        tctx.translate(sourceW, 0);
        tctx.scale(-1, 1);
      }
      tctx.drawImage(video, 0, 0, sourceW, sourceH);
      try {
        const out = lens
  ...
  const toggleResultWatermark = async () => {
    if (!resultUrl || !sourceUrl || !lens) return;

    const next = !wantWm;
    setWantWm(next);

    // Free users must retain the Motio2edit Lenses watermark.
    if (!isPaid) {
      setWantWm(true);
      return;
    }
    ...
    <div className="relative flex h-[100dvh] flex-col overflow-hidden bg-black text-white">
      ...
      >
        ...
        {phase === "processing" && (
          <div className="pointer-events-none absolute inset-0 z-40 grid place-items-center bg-black/35">
            <div className="lens-aperture" aria-hidden="true">
              <span />
              <span />
              <span />
              <span />
          ...
          <div className="flex flex-wrap gap-3">
            ...
            <button
              ...
              className={cn(
                ...
                wantWm ? "bg-white text-black" : "bg-white/15 text-white",
              )}
              onClick={() => void toggleResultWatermark()}
            >
              {wantWm ? "Watermark On" : "Watermark Off"}
            </button>
            <button
              type="button"
      ...
      <style>{`
        .lens-aperture {
          ...
          animation: lens-aperture-pulse .72s cubic-bezier(.4,0,.2,1) both;
        }
        .lens-aperture span {
          position: absolute;
          left: 50%;
          ...
          animation: lens-aperture-blade .72s cubic-bezier(.4,0,.2,1) both;
        }
        .lens-aperture span:nth-child(1) { transform: translate(-50%,-100%) rotate(0deg); }
        .lens-aperture span:nth-child(2) { transform: translate(-50%,-100%) rotate(60deg); }
        .lens-aperture span:nth-child(3) { transform: translate(-50%,-100%) rotate(120deg); }
        .lens-aperture span:nth-child(4) { transform: translate(-50%,-100%) rotate(180deg); }
        .lens-aperture span:nth-child(5) { transform: translate(-50%,-100%) rotate(240deg); }
        .lens-aperture span:nth-child(6) { transform: translate(-50%,-100%) rotate(300deg); }
        @keyframes lens-aperture-blade {
          0% { opacity:.9; scale:1; }
          45% { opacity:1; scale:.58; }
          100% { opacity:.9; scale:1; }
        }
        @keyframes lens-aperture-pulse {
          0% { scale:1; opacity:0; }
          12% { opacity:1; }
          50% { scale:.92; }
