...
export function LensEditor({ initialLensId }: { initialLensId?: string }) {
  ...
  const toggleWatermark = async () => {
    ...
    try {
      ...
      ctx.imageSmoothingQuality = "high";
      ctx.drawImage(img, 0, 0);
      const final = applyLensOpticalEnhanced(c, lens, "native", { watermark: next, maxEdge: 2560 });
    ...
    <div className="relative flex h-[100dvh] flex-col overflow-hidden bg-black text-white">
      ...
      <div
        className={cn(
          ...
          isAiLens(lens) && phase !== "result" && "ring-2 ring-amber-400/70 shadow-[0_0_40px_rgba(251,191,36,0.35)]",
        )}
        onTouchStart={(e) => {
