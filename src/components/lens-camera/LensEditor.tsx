...
export function LensEditor({ initialLensId }: { initialLensId?: string }) {
  ...
  useEffect(() => {
    // Apply lens to uploaded image immediately (no extra tap required)
  ...
  const onPick = (file: File | null) => {
    ...
    // Show original immediately; lens preview effect fills previewUrl as soon as image decodes
    setSourceUrl(url);
    setPreviewUrl(null);
    setResultUrl(null);
    setPhase("ready");
    ...
    <div className="relative flex h-[100dvh] w-full flex-col overflow-hidden bg-black text-white">
      ...
      >
        <div className="absolute inset-0 flex items-center justify-center overflow-hidden">
          <video
            ...
            autoPlay
            className={cn(
              "pointer-events-none absolute left-1/2 top-1/2 h-full w-full max-w-none object-cover object-center",
          ...
          {showStill && stillSrc && (
            <img
              ...
              alt="Preview"
              className={cn(
                "pointer-events-none absolute left-1/2 top-1/2 h-full w-full max-w-none object-center",
                phase === "result" ? "object-contain" : "object-cover",
              )}
      ...
      <div className="absolute bottom-0 left-0 right-0 z-30 pb-[max(0.75rem,env(safe-area-inset-bottom))]">
        <div className="bg-gradient-to-t from-black via-black/90 to-transparent pt-16">
          <div
            ref={carouselRef}
            className="flex items-end gap-3 overflow-x-auto px-[calc(50%-2.25rem)] pb-3 scrollbar-none"
              ...
              return (
                <button
                  ...
                  className={cn(
                    ...
                    active
                      ? "h-[4.5rem] w-[4.5rem] scale-105 border-white opacity-100"
                      : "h-[3.75rem] w-[3.75rem] border-white/25 opacity-55",
                  )}
                  style={{ scrollSnapAlign: "center" }}
                >
                  {sample ? (
                    <img src={sample} alt={l.name} className="h-full w-full object-cover" draggable={false} />
                  ) : (
                    <div
