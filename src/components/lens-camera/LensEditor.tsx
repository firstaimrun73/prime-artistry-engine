...
export function LensEditor({ initialLensId }: { initialLensId?: string }) {
  ...
  return (
    <div className="relative flex h-[100dvh] flex-col overflow-hidden bg-black text-white">
      <header className="absolute left-0 right-0 top-0 z-30 flex items-center justify-between px-3 pt-[max(0.75rem,env(safe-area-inset-top))] pb-2">
        <div className="flex items-center gap-2">
          ...
          <Link
            ...
            className="rounded-full bg-black/40 px-3 py-2 text-[11px] font-semibold tracking-wide backdrop-blur-md"
      ...
      <div
        className={cn(
          "absolute inset-0 z-0 overflow-hidden touch-pan-y",
          isAiLens(lens) && phase !== "result" && "ring-2 ring-amber-400/80 shadow-[0_0_24px_rgba(251,191,36,0.45)]",
        )}
        onTouchStart={(e) => {
      ...
      >
        <video
          ...
          className={cn(
            ...
            showCamera && !liveFxOn ? "opacity-100" : "opacity-0 pointer-events-none",
          )}
        ...
        {showStill && stillSrc && (
          <img
            src={stillSrc}
            alt="Preview"
            className={`absolute inset-0 h-full w-full ${phase === "result" ? "object-contain bg-black" : "object-cover"}`}
          />
        )}
        {phase === "processing" && (
        ...
        {apertureOn && (
          <div className="pointer-events-none absolute inset-0 z-[25] flex items-center justify-center bg-black/40">
            <div className="relative h-44 w-44">
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <div
                  key={i}
                  className="absolute left-1/2 top-1/2 h-[72%] w-[20%] origin-[50%_100%] rounded-[2px] bg-zinc-950 shadow-md"
                  style={{
      ...
      {/* Bottom controls — overlays only; do not shrink camera */}
            ...
            {ORDERED_ROSTER.map((l) => {
              ...
              return (
                <button
                  ...
                  className={cn(
                    "relative shrink-0 scroll-snap-center",
                    "h-16 w-16 overflow-hidden rounded-full border-2 transition",
                    active ? "border-white scale-110" : "border-white/30 opacity-80",
                  )}
                >
                  {sample ? (
                    <img src={sample} alt={l.name} className="h-full w-full object-cover" />
                  ) : (
                    <div className="grid h-full w-full place-items-center bg-zinc-800 text-[10px] font-bold">
                      {l.name.slice(0, 2)}
                    </div>
                  )}
                  {l.tier === "ai" && (
                    <span className="absolute bottom-0 left-0 right-0 bg-orange-500/90 text-[8px] font-bold text-white">
                      AI
                    </span>
                  )}
                </button>
          ...
          <div className="flex items-center justify-between px-6 pb-3">
            ...
            {phase === "result" ? (
              ...
              <button
                ...
                className="grid place-items-center rounded-full border-4 border-white bg-white/20 active:scale-95"
      ...
      {infoOpen && lens && (
        ...
        >
          <div
            className="m-4 max-h-[70dvh] w-full max-w-sm overflow-y-auto rounded-2xl border border-white/15 bg-zinc-900/95 p-5 shadow-xl backdrop-blur-md"
            onClick={(e) => e.stopPropagation()}
          >
            <div className="mb-3 flex items-start justify-between gap-3">
              <div>
                <p className="text-lg font-bold">{lens.name}</p>
                <p className="text-xs text-white/60">
                  {isAiLens(lens) ? "AI+ · result after capture" : "Common · free optical"}
                </p>
              </div>
              <button
                type="button"
            ...
            {isAiLens(lens) && (
              <p className="mt-3 text-xs text-amber-300/90">
                AI+ uses plan entitlement (5 successful generations per day). No live backend preview —
                the enhanced result appears after you capture or upload.
              </p>
            )}
          </div>
        </div>
