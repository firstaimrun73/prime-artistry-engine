...
export function LensEditor({ initialLensId }: { initialLensId?: string }) {
  ...
  const carouselSlots = useMemo(() => {
    ...
    for (let off = -2; off <= 2; off++) {
      ...
      slots.push({ lens: l, key: `${l.id}-off${off}`, offset: off });
    }
    return slots;
  }, [activeIndex]);
  ...
  const toggleWatermark = async () => {
    if (!resultUrl || !lens || !sourceUrl) return;
    setWantWm((v) => !v);
    const nextWm = !wantWm;
    try {
      const img = await loadImage(sourceUrl);
  ...
  return (
    <div className="relative h-[100dvh] w-full overflow-hidden bg-black text-white">
      <style>{`
        ...
        .m2e-ai-glow { animation: m2e-ai-glow-pulse 2.4s ease-in-out infinite; }
      `}</style>
              ...
              {[0, 1, 2, 3, 4, 5].map((i) => (
                <div
                  ...
                  style={{
                    ...
                    height: "52%",
                    marginLeft: -4,
                    transform: `translate(-50%, -100%) rotate(${i * 60}deg) scaleY(1)`,
                    animation: `m2e-blade-${i} 0.42s ease-out forwards`,
                  }}
                />
              ))}
            </div>
      ...
      <div className="absolute bottom-0 left-0 right-0 z-30 pb-[max(0.5rem,env(safe-area-inset-bottom))]">
        ...
        <div className="relative mx-auto flex w-full max-w-md flex-col items-center px-2">
          ...
          {/* Active lens name — fixed height so layout never jumps */}
          <div className="relative z-20 mb-1 flex h-6 items-center justify-center">
            {lens && phase !== "result" && (
              <p
                key={lens.id}
                className="text-center text-[13px] font-semibold tracking-wide text-white/95 animate-in fade-in duration-200"
              >
                {lens.name}
                {isAiLens(lens) ? (
          ...
          {/* Unified camera-control zone: floating 5-slot lens arc + shutter */}
          <div className="relative z-10 flex w-full flex-col items-center">
            {/* Lens bubbles — absolute arc overlapping shutter visually */}
            {phase !== "result" && (
              <div
                ref={carouselRef}
                className="relative z-20 h-[4.75rem] w-full max-w-[20rem]"
              ...
              >
                {carouselSlots.map((slot, slotIdx) => {
                  ...
                  // Depth hierarchy: outer 0.72 / adjacent 0.88 / active 1.15
                  const opacity = isCenter ? 1 : absOff === 1 ? 0.78 : 0.52;
                  // Base sizes encode the scale hierarchy (no extra CSS scale needed for layout stability)
                  const sizeRem = isCenter ? 4.0 : absOff === 1 ? 3.05 : 2.5;
                  // Horizontal positions relative to center (%)
                  const xPct = off * 21.5;
                  // Vertical: center elevated, sides slightly lower
                  const yPx = isCenter ? -2 : absOff === 1 ? 12 : 18;
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        "absolute left-1/2 top-0 overflow-hidden rounded-full transition-all duration-[220ms] ease-[cubic-bezier(0.34,1.3,0.64,1)] will-change-transform",
                        isCenter
                          ? "border-[2.5px] border-white shadow-[0_0_0_3px_rgba(255,255,255,0.25),0_0_16px_rgba(255,255,255,0.15),0_4px_18px_rgba(0,0,0,0.45)]"
                      ...
                      style={{
                        ...
                        height: `${sizeRem}rem`,
                        opacity,
                        transform: `translate(calc(-50% + ${xPct}%), ${yPx}px)`,
                        zIndex: z,
                        pointerEvents: "auto",
                      ...
                      {sample ? (
                        <img
                          ...
                          className={cn(
                            ...
                            !isCenter && "brightness-75 contrast-95",
                          )}
                          draggable={false}
                        />
                      ) : (
            ...
            {/* Shutter — visually under active lens, dominant, high z for tap */}
