 ...
 *
 * Source image persists across lens changes. Output-only watermark.
...
export function LensEditor({ initialLensId }: { initialLensId?: string }) {
  ...
  useEffect(() => {
    ...
    const email = String((user as { email?: string } | null)?.email ?? "").toLowerCase();
    // Client UI gate only — server still enforces paid/admin for AI+.
    const adminUi = email === "firstaimrun89@gmail.com";
    const paidUi = adminUi || (!!plan && plan !== "free");
    setIsPaid(paidUi);
    if (!paidUi) setWantWm(true);
  }, [user]);
  ...
  // PLACEHOLDER_CONTINUED - file too large for single tool call
  return null;
}
