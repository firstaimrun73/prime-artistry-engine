    ...
    <div className="relative flex min-h-[100dvh] flex-col bg-zinc-100 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-50">
      ...
      <div className="relative flex flex-1 items-center justify-center overflow-hidden px-3 pb-48 pt-16">
        ...
        {(phase === "ready" || phase === "processing" || phase === "result") && stillSrc && (
          <div className="relative w-full max-w-lg overflow-hidden rounded-3xl border border-white/40 bg-black/5 shadow-xl backdrop-blur-sm dark:border-white/10">
            <img
              src={stillSrc}
            ...
            {phase === "result" && (
              <button
                type="button"
                className="absolute right-2 top-2 z-10 rounded-full border border-white/30 bg-black/55 px-3 py-1.5 text-[10px] font-semibold text-white shadow-md backdrop-blur-md active:scale-95"
                onPointerDown={(e) => {
                  e.preventDefault();
                  e.stopPropagation();
              ...
              >
                {holdingOriginal ? "Original" : "Hold original"}
              </button>
            )}
            {phase === "ready" && !lens && (
            ...
            {phase === "ready" && lens && previewUrl && (
              <p className="absolute bottom-2 left-1/2 -translate-x-1/2 rounded-full border border-white/15 bg-black/40 px-3 py-1 text-[10px] text-white/90 backdrop-blur-md">
                Preview · shutter to commit{isAiLens(lens) ? ` · ${lens.creditCost} cr` : " · free"}
              </p>
            )}
            {phase !== "result" && (
              <button
      ...
      <div className="absolute inset-x-0 bottom-0 z-30 space-y-3 bg-gradient-to-t from-zinc-100 via-zinc-100/90 to-transparent px-3 pb-[max(1rem,env(safe-area-inset-bottom))] pt-10 dark:from-zinc-950 dark:via-zinc-950/90">
          ...
          {CAMERA_LENS_ROSTER.map((l) => {
            ...
            return (
              ...
              >
                <span
                  className={cn(
                    "relative grid h-[54px] w-[54px] place-items-center overflow-hidden rounded-full text-[11px] font-bold text-white shadow-md transition",
                    active
                      ? "scale-110 ring-2 ring-amber-400/90 ring-offset-2 ring-offset-zinc-100 dark:ring-offset-zinc-950"
                      : "opacity-95",
                  )}
                  style={{
                    background: `linear-gradient(145deg, ${l.color}cc, ${l.color}99)`,
                    boxShadow: active
                      ? `0 0 18px ${l.color}99, inset 0 1px 0 rgba(255,255,255,0.55)`
                      : `0 4px 14px ${l.color}55, inset 0 1px 0 rgba(255,255,255,0.4)`,
                ...
                >
                  <span
                    className="pointer-events-none absolute inset-x-1 top-0.5 h-1/2 rounded-full opacity-50"
                    style={{
                      background: "linear-gradient(180deg, rgba(255,255,255,0.55), transparent)",
                    }}
                  />
                  <span className="relative z-[1] drop-shadow-sm">{l.code}</span>
                  {l.tier === "ai" && (
                    <span className="absolute -bottom-0.5 left-1/2 z-[1] -translate-x-1/2 rounded-full bg-amber-500/95 px-1 text-[7px] font-bold leading-3 text-white">
                      AI
                    </span>
                  )}
                </span>
        ...
        <div className="flex items-center justify-center gap-8">
          ...
          <button
            ...
            style={{
              background: "linear-gradient(145deg, rgba(255,220,120,0.55), rgba(245,158,11,0.35))",
              boxShadow:
        ...
        <p className="pb-1 text-center text-[10px] text-muted-foreground">
          Upload · pick lens · shutter · AI lenses 20 cr · free lenses watermarked
        </p>
      </div>
