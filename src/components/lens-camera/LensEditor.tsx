...
export function LensEditor({ initialLensId }: Props) {
  ...
  // Live / static preview (never watermarked)
  useEffect(() => {
    ...
    const run = async () => {
      ...
      try {
        ...
        // Preview: watermark = false always
  ...
  const applyFromCanvas = async (canvas: HTMLCanvasElement, active: CameraLensDef) => {
    ...
    setPhase("processing");
    try {
      // Final apply: watermark controlled by wantWm (and paid status)
  ...
  // Detect paid plan roughly from user object if available
  useEffect(() => {
    const plan =
    ...
    if (!plan || plan === "free") setWantWm(true); // free always watermarked
  }, [user]);
  ...
  return (
    <div className="relative flex min-h-[100dvh] flex-col bg-black text-white">
      ...
      <div className="relative flex flex-1 items-center justify-center overflow-hidden px-2 pb-44 pt-14">
          ...
          <div className="relative w-full max-w-lg overflow-hidden rounded-3xl border border-white/10 bg-black/40">
            {showResult ? (
              <CompareSlider
                before={sourceUrl!}
                after={resultUrl!}
                className="mx-auto max-h-[min(58dvh,600px)] w-auto"
              />
            ) : (
              <img
                src={stillSrc}
        ...
        {/* Brief iOS-style name chip (lens change only) */}
        {nameChip && phase !== "result" && (
          <div className="pointer-events-none absolute left-1/2 top-[max(4.5rem,env(safe-area-inset-top)+3.5rem)] z-30 -translate-x-1/2 animate-in fade-in zoom-in-95 duration-200">
      ...
      {/* Bottom bar — floating circles + fixed shutter */}
        ...
        {showResult ? (
          ...
          <div className="mx-auto flex max-w-md flex-col items-center gap-3 px-4">
            ...
            <button
              ...
              className={cn(
                "flex items-center gap-1.5 rounded-full border px-4 py-1.5 text-[11px] font-medium backdrop-blur-xl",
                isPaid
          ...
          <>
            {/* Floating circular lens previews — no names */}
            <div
              ref={carouselRef}
