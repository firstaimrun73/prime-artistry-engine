...
export function EditorPromptPanel({
  ...
  const insertTag = (tagId: string) => {
    ...
    if (onToggleTag && !contextTags.includes(tagId)) onToggleTag(tagId);
    setAtMenuOpen(false);
    requestAnimationFrame(() => taRef.current?.focus());
  };

  const removeChip = (id: string) => { onToggleTag?.(id); };
  ...
  return (
    <>
      {mediaType === "image" && activeToolLabel && (
        <section className="min-w-0 space-y-2">
          <div className="flex min-w-0 items-center gap-2 rounded-lg border border-primary/30 bg-primary/5 px-3 py-2 text-xs">
            ...
            {onClearTool && (
              <button type="button" onClick={onClearTool} className="ml-auto inline-flex shrink-0 items-center gap-1 text-muted-foreground hover:text-foreground">
      ...
      {mediaType === "video" && (
        <section className="min-w-0 space-y-2">
          <p className="text-[11px] font-semibold uppercase tracking-wide text-muted-foreground">Quick styles</p>
          <div className="flex flex-wrap gap-2">
            {VIDEO_QUICK_STYLES.map((q) => (
              <button key={q.label} type="button" onClick={() => setPrompt(q.prompt)} className="btn-animate min-h-[40px] rounded-full border border-border bg-card px-3 py-2 text-xs text-muted-foreground hover:border-primary hover:text-foreground">
          ...
          <div className="flex min-w-0 flex-wrap items-center gap-1.5 px-2.5 py-1.5 sm:px-3 sm:py-2">
            {contextHint && (
              <span className="inline-flex shrink-0 items-center gap-1 text-[10px] font-medium tracking-wide text-muted-foreground sm:text-[11px]">
                <span className="text-primary/80" aria-hidden>✦</span>{contextHint}
              </span>
            )}
            {contextTags.map((id) => {
              ...
              if (!tag) return null;
              return (
                <span key={id} className={cn("inline-flex max-w-full items-center gap-0.5 rounded-full border px-1.5 py-0.5 text-[10px] font-semibold sm:px-2", tag.color)}>
                  {tag.label}
                  <button type="button" className="opacity-70 hover:opacity-100" onClick={() => removeChip(id)} aria-label={`Remove ${tag.label}`}>
                    <X className="h-2.5 w-2.5" />
                  </button>
                </span>
              );
            })}
            <span className="ml-auto hidden text-[10px] text-muted-foreground/70 sm:inline">Type @ for tags</span>
          </div>

          <div className="relative min-w-0 px-2.5 pt-1 pb-1.5 sm:px-3 sm:pb-2">
            <Textarea
              ref={taRef}
              placeholder={totalImages >= 1 ? "Describe the edit… (type @ for tags)" : "Describe what you want to create… (type @ for tags)"}
              value={prompt}
            ...
            <div className="absolute right-1.5 top-1.5 z-10 flex max-w-[calc(100%-0.75rem)] items-start justify-end sm:right-2 sm:top-2">
              <VoiceInputButton disabled={loading} onTranscript={(txt) => setPrompt((p) => (p ? `${p} ${txt}` : txt).slice(0, limit))} />
            </div>

            {atMenuOpen && atMatches.length > 0 && (
              <div ref={menuRef} role="listbox" className="absolute left-2 right-2 z-20 mt-0.5 max-h-44 overflow-y-auto rounded-lg border border-border/80 bg-popover p-1 shadow-lg sm:left-3 sm:right-14">
                {atMatches.map((tag) => (
                  <button key={tag.id} type="button" role="option" onMouseDown={(e) => { e.preventDefault(); insertTag(tag.id); }} className="flex w-full items-center gap-2 rounded-md px-2 py-1.5 text-left text-xs font-semibold transition-colors hover:bg-muted">
                    <span className={cn("rounded-full border px-1.5 py-0.5 text-[10px]", tag.color)}>{tag.label}</span>
                    <span className="truncate text-[10px] font-normal text-muted-foreground">{tag.guidance}</span>
                  </button>
                ))}
              </div>
            )}
        ...
        {!loading && suggestions.length > 0 && (
          <div className="-mx-0.5 flex gap-1.5 overflow-x-auto px-0.5 pb-0.5 scrollbar-none">
            {suggestions.slice(0, 3).map((s) => (
              <button key={s.label} type="button" onClick={() => setPrompt(s.prompt)} className="min-h-[32px] shrink-0 rounded-full border border-primary/30 bg-primary/5 px-2.5 py-1 text-[11px] font-medium text-primary transition-colors hover:bg-primary/10">{s.label}</button>
            ))}
          </div>
        )}
