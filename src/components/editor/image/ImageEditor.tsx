...
export function ImageEditor({ bootstrap }: ImageEditorProps) {
  ...
  // Paid: refs when plan allows >1. Free: still show section so lock UI is visible (adds blocked inside).
  const canAddRefImages = !!inputDataUrl && (planLimits.maxImages > 1 || isFree);
  ...
  const onFile = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Free (non-admin): hard-cap at 1 image — no multi-reference via gallery/picker.
    const maxAllowed = isAdmin ? planLimits.maxImages : Math.min(planLimits.maxImages, isFree ? 1 : planLimits.maxImages);
  ...
  const runImageJob = async (opts: {
    ...
    try {
      ...
      if (!maskImageUrl && planLimits.maxImages > 1 && refImages.length > 0) {
          ...
          <div className="order-1 min-w-0 space-y-4 sm:space-y-5">
            <div className={cn("min-w-0 space-y-3 p-3 sm:p-5", studioCardClass(studioTier))}>
              ...
              <EditorUpload
                ...
                maxGalleryImages={Math.min(MAX_GALLERY_IMAGES, isFree ? 1 : planLimits.maxImages)}
            ...
            <div className={cn("min-w-0 space-y-3 p-3 sm:p-5", studioCardClass(studioTier))}>
              ...
              <EditorPromptPanel
                ...
                onToggleTag={(id) => {
                  ...
                  // EditorPromptPanel.insertTag already strips the typed @query range.
                  setContextTags((prev) =>
