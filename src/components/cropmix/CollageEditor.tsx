...
import {
  ...
  CROPMIX_TIMEOUT_MS,
  CROPMIX_VOLT,
  type CollageCanvasRatio,
  type CollageCell,
  type CollageCustomize,
  type CollageStyleId,
  type PhotoSlot,
...
export function CollageEditor({ onResult, onCancel }: Props) {
  ...
  const cells: CollageCell[] = useMemo(
    () =>
      photos.map((p) => ({
        photoId: p.id,
        fit: "fill" as const,
  ...
  const handleExport = async () => {
    ...
    try {
      if (isAiPlus) {
        ...
        try {
          ...
          const msg =
            e instanceof Error
              ? e.message
              : "AI+ collage failed. No credits charged.";
          toast.error(msg);
        }
      } else {
      ...
      toast.error(
        e instanceof Error ? e.message : "Could not export collage.",
      );
    } finally {
      setBusy(false);
    }
    ...
    <div className="flex h-full flex-col bg-background">
      ...
      <div className="relative flex min-h-0 flex-1 items-center justify-center overflow-hidden bg-black/90 p-3">
        {previewUrl ? (
          ...
          <p className="text-sm text-muted-foreground">
            {photos.length ? "Building preview…" : "Add photos to begin"}
          </p>
        )}
      </div>
      ...
      <div className="flex shrink-0 gap-2 overflow-x-auto border-t border-border px-3 py-2">
        {COLLAG_STYLES.map((s) => {
          const disabled =
            photos.length > 0 &&
            (photos.length < s.cellCount.min || photos.length > s.cellCount.max);
          ...
          return (
            <button
              ...
              className={cn(
                "shrink-0 rounded-full border px-3 py-1 text-xs font-medium disabled:opacity-40",
                active
        ...
        <div className="shrink-0 space-y-3 border-t border-border bg-card px-4 py-3">
          ...
          {style.capabilities.gutter && (
            <label className="flex items-center justify-between gap-3 text-sm">
              ...
              <input
                ...
                min={0}
                max={40}
                value={customize.gutter ?? style.defaults.gutter}
                onChange={(e) =>
                  setCustomize((c) => ({ ...c, gutter: Number(e.target.value) }))
                }
              />
            </label>
          )}
          {style.capabilities.border && (
            <label className="flex items-center justify-between gap-3 text-sm">
              ...
              <input
                ...
                min={0}
                max={12}
                value={customize.border ?? style.defaults.border}
                onChange={(e) =>
                  setCustomize((c) => ({ ...c, border: Number(e.target.value) }))
                }
              />
            </label>
          )}
          {style.capabilities.cornerRadius && (
            <label className="flex items-center justify-between gap-3 text-sm">
              ...
              <input
                ...
                min={0}
                max={32}
                value={customize.cornerRadius ?? style.defaults.cornerRadius}
                onChange={(e) =>
                  setCustomize((c) => ({
                    ...c,
          ...
          {style.capabilities.background && (
            <label className="flex items-center justify-between gap-3 text-sm">
              ...
              <input
                type="color"
                value={customize.background ?? style.defaults.background}
                onChange={(e) =>
                  setCustomize((c) => ({ ...c, background: e.target.value }))
                }
              />
            </label>
          )}
